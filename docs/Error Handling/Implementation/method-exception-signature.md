...
Thrown exceptions do not behave like returned values.  The possible exceptions that a method might throw is the UNION of all the types of exceptions that can be thrown from all the methods it calls, and all the methods those call, ad nausium.  Let me give a clear
...
NOTE: this recommendation has changed, and instead you should never declare a thows declaration.  See [Say ‘No’ to Checked Exception](https://agiletribe.purplehillsbooks.com/2023/07/13/say-no-to-checked-exceptions/).
